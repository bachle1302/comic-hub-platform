...
import { backendLoginResultSchema } from "@/features/auth/api/auth.schema";
import {
...
export async function POST(request: NextRequest) {
  ...
  const backendResponse = await fetch(getBackendAuthUrl("/auth/login"), {
    method: "POST",
    cache: "no-store",
    headers: {
  ...
  try {
    const data = parseBackendSuccess(payload, backendLoginResultSchema);
